...
router.post('/editar_pedido/:id', isLoggedIn, async function(req, res){
	...
	await pedido.findById(id, async function(err, pedido){
		let numero_pedido = pedido.numero_pedido;
		await crearPedido.findByIdAndUpdate(req.parmas.id,{cliente_nombre: cliente_nombre, cliente_telefono: cliente_telefono, sucursal: sucursal, descripción: descripcion, estado: estado, total: total}, async function (err) {
